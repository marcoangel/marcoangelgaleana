<!-- Skills-->
<section class="resume-section" id="skills">
    <div class="resume-section-content">
        <h2 class="mb-5">Habilidades </h2>
        <div class="subheading mb-3">Herramientas y lenguajes de programacion</div>
        <ul class="list-inline dev-icons">
            <li class="list-inline-item"><i class="fab fa-linux"></i></li>
            <li class="list-inline-item"><i class="fab fa-java"></i></li>
            <li class="list-inline-item"><i class="fab fa-git"></i></li>
            <li class="list-inline-item"><i class="fab fa-php"></i></li>
            <li class="list-inline-item"><i class="fab fa-cuttlefish"></i></li>
            <li class="list-inline-item"><i class="fas fa-database"></i></li>
            <li class="list-inline-item"><i class="fas fa-asterisk"></i></li>
            <li class="list-inline-item"><i class="fas fa-tty"></i></li>
            <li class="list-inline-item"><i class="fas fa-broadcast-tower"></i></li>
            <li class="list-inline-item"><i class="fas fa-network-wired"></i></li>
            <li class="list-inline-item"><i class="fas fa-wifi"></i></li>
        </ul>
        <div class="subheading mb-3">Certificados</div>
        <ul class="fa-ul mb-0">
            <li>
                <span class="fa-li"><i class="fas fa-check"></i></span>
                Diploma en Gestión de la Ciberseguridad y Marco Legal
            </li>
            <li>
                <span class="fa-li"><i class="fas fa-check"></i></span>
                Diploma en Seguridad en Sistemas, Aplicaciones y Auditoría
            </li>
            <li>
                <span class="fa-li"><i class="fas fa-check"></i></span>
                Diploma en Tecnologías de Identificación Digital e Investigación
            </li>
            <li>
                <span class="fa-li"><i class="fas fa-check"></i></span>
                Agile Development & Scrum
            </li>
        </ul>
    </div>
</section>
<hr class="m-0" />